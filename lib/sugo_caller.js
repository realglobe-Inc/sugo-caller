...
class SugoCaller extends SugoClient {
  ...
  async connect (key, options = {}) {
    ...
    const doConnect = async () => {
      ...
      let access
      try {
        access = await s.actorAccess(key, messages)
      } catch (err) {
  ...
  newSocket (options = {}) {
    const s = this
    const {url, multiplex, path} = s
    return sgSocketClient(url, Object.assign({multiplex, path}, options))
  }
  ...
  async actorAccess (key, messages) {
    ...
    const connector = s.newConnector(key)
          ...
          let handleResult = (data) => {
            ...
            switch (status) {
              case NG: {
                ...
                const error = Object.assign(new Error(message), payload, additionalInfo)
                reject(error)
                break
              }
              case OK:
